#!/bin/bash
# ArtOnly — Burna Boy: enable publishing + publish article 2026-06-26
# sshpass -p 'Neverending48!' ssh -o PreferredAuthentications=password -o StrictHostKeyChecking=no dh_yadmw3@pdx1-shared-a2-06.dreamhost.com 'bash -s' < deploy-artonly-2026-06-26-burna-boy.sh

echo "=== ArtOnly Deploy: Burna Boy 2026-06-26 ==="
echo "$(date)"
...
curl -s -L -A "Mozilla/5.0 (X11; Linux x86_64)" \
  -o "$IMG_DIR/${SLUG}-hero.jpg" \
  "https://upload.wikimedia.org/wikipedia/commons/c/ce/Untold_2024_-Burna_Boy_%2853926047977%29_%28cropped%29.jpg"
echo "hero: $(wc -c < $IMG_DIR/${SLUG}-hero.jpg) bytes"

# Body images with fallback check
declare -A IMGS
IMGS[intro]="https://upload.wikimedia.org/wikipedia/commons/0/01/Burna_Boy.jpg"
IMGS[live]="https://upload.wikimedia.org/wikipedia/commons/6/67/Untold_2024_-Burna_Boy_%2853926941096%29.jpg"
IMGS[studio]="https://upload.wikimedia.org/wikipedia/commons/8/8f/Untold_2024_-Burna_Boy_%2853927293629%29.jpg"
IMGS[session]="https://upload.wikimedia.org/wikipedia/commons/e/ef/Burna_Boy_%28cropped%29.jpg"
IMGS[promo]="https://upload.wikimedia.org/wikipedia/commons/5/52/Afrobeats_star_Burna_Boy_performing_with_at_Nativeland_Concert%2C_Lagos%2C_Nigeria_2016.jpg"

for name in intro live studio session promo; do
    url="${IMGS[$name]}"
    dest="$IMG_DIR/${SLUG}-${name}.jpg"
    curl -s -L -A "Mozilla/5.0 (X11; Linux x86_64)" -o "$dest" "$url"
    sz=$(wc -c < "$dest")
    if [ "$sz" -ge 5000 ]; then
        echo "$name: ${sz} bytes OK"
    else
        echo "$name: ${sz} bytes FAIL — removing file"
        rm -f "$dest"
    fi
done
...
payload = {
    ...
    'slug': SLUG,
    'artist': 'Burna Boy',
    'image_url': 'https://upload.wikimedia.org/wikipedia/commons/c/ce/Untold_2024_-Burna_Boy_%2853926047977%29_%28cropped%29.jpg',
    'post': {
        'title': 'Burna Boy and the Album That Made Africa Non-Negotiable',
        'date': '2026-06-26',
        'category': 0,
        'pen_name': 'Allastair Voss',
        'body': body,
        'crew': [

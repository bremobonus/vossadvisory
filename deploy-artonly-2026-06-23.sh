...
payload = json.dumps({
    ...
    "subject": "ArtOnly Album Monitor: 2026-06-23",
...
proc = subprocess.run(
    ['python3', 'artonly.io/api/gmail-send.py'],
    input=payload, capture_output=True, text=True
)
print(proc.stdout or proc.stderr, flush=True)
if proc.returncode != 0:
    print(f"ERROR: gmail-send.py exited {proc.returncode}", file=sys.stderr)
    sys.exit(1)
PYEOF

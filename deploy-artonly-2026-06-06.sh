...
# Try Stereogum
try:
    html = fetch('https://www.stereogum.com/2292000/death-cab-for-cutie-i-built-you-a-tower-review/music/')
    imgs = re.findall(r'(https?://[^\s\"\'<>]+\.(?:jpg|jpeg))', html, re.IGNORECASE)
    imgs = [i for i in imgs if 'death' in i.lower() or 'cab' in i.lower() or 'gibbard' in i.lower() or 'tower' in i.lower()]
    if not imgs:
...
# Try Mixtape Madness
try:
    html = fetch('https://www.mixtapemadness.com/blog/music/vince-staples-returns-with-new-album-cry-baby')
    imgs = re.findall(r'(https?://[^\s\"\'<>]+\.(?:jpg|jpeg))', html, re.IGNORECASE)
    imgs = [i for i in imgs if 'vince' in i.lower() or 'staples' in i.lower()]
    if imgs:
        download_img(imgs[0], dest, referer='https://www.mixtapemadness.com/')
        print('Downloaded from Mixtape Madness')
        sys.exit(0)
except Exception as e:
    print(f'Mixtape Madness failed: {e}')

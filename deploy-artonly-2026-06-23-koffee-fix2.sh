#!/bin/bash
# ArtOnly: Original Koffee - FIX2: diagnose stored body + move img to top
# Run via GitHub Actions (deploy-artonly.yml)
set -e
cd /home/dh_yadmw3

echo "=== Diagnose: read currently stored body ==="
python3 << 'PYEOF'
import json, os

path = 'artonly.io/posts/original-koffee.json'
if os.path.exists(path):
    ...
    h2_pos = body.find('## ')
    print(f"Stored body length: {len(body)}", flush=True)
    print(f"Stored [img:] position: {img_pos}", flush=True)
    print(f"Stored ## position: {h2_pos}", flush=True)
    print(f"First 1200 chars of stored body:\n{body[:1200]}", flush=True)
else:
    print(f"ERROR: {path} not found", flush=True)
PYEOF

echo "=== Republish: img placed after youtube, before intro text ==="
python3 << 'PYEOF'
...
print(f"koffee-intro.jpg exists={has('koffee-intro.jpg')}", flush=True)
...
print(f"[img:] at position {img_pos}, ## at {heading_pos}", flush=True)
...
echo "=== Diagnose: read stored body AFTER publish ==="
python3 << 'PYEOF'
import json, os
...
img_pos = body.find('[img:')
h2_pos = body.find('## ')
print(f"After-publish body length: {len(body)}", flush=True)
print(f"After-publish [img:] position: {img_pos}", flush=True)
print(f"After-publish ## position: {h2_pos}", flush=True)

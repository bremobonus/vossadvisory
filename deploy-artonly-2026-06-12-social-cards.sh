...
# Purpose: Ensure every published post has a canonical social card.
#   - Audits all posts in ~/artonly.io/posts/
#   - Copies hero image to artists dir if needed
#   - Runs make-social-card.py (OMP_NUM_THREADS=1) for each missing card
#   - Appends [download:] tag to post body and saves JSON
#   - Fixes Rule 5 (hero image repeated as first [img:] in body)
#   - Fixes wrong-format download tags (missing /assets/images/artists/ prefix)
# Run via GitHub Actions (deploy-artonly.yml) or:
# sshpass -p 'Neverending48!' ssh -o PreferredAuthentications=password -o StrictHostKeyChecking=no dh_yadmw3@pdx1-shared-a2-06.dreamhost.com 'bash -s' < deploy-artonly-2026-06-12-social-cards.sh

set -e
echo "=== ArtOnly Social Card Audit: 2026-06-12 ==="
echo "$(date)"
echo ""
...
for p in sorted(glob.glob(f'{POSTS_DIR}/*.json')):
    ...
    # Rule 4: check/fix download tag format
    ...
    if has_correct_dl:
        ...
        # Missing download tag: ensure hero, generate card, append tag
        hero_path = f'{IMAGES_DIR}/{slug}.jpg'
        if not os.path.exists(hero_path):
            src_path = os.path.expanduser('~/artonly.io' + hero) if hero else ''
            if src_path and os.path.exists(src_path):

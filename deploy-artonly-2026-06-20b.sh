#!/bin/bash
# ArtOnly Deploy: Future article
# Generated: 2026-06-20
# Run via GitHub Actions (deploy-artonly.yml) or:
# sshpass -p 'Neverending48!' ssh -o PreferredAuthentications=password -o StrictHostKeyChecking=no dh_yadmw3@pdx1-shared-a2-06.dreamhost.com 'bash -s' < deploy-artonly-2026-06-20b.sh

set -e
cd /home/dh_yadmw3
echo "=== ArtOnly: Future article deploy: $(date) ==="
...
curl -s -L -A "$UA" \
  -o "artonly.io/assets/images/artists/future.jpg" \
  "https://upload.wikimedia.org/wikipedia/commons/9/93/Future_-_Openair_Frauenfeld_2019_01_%28cropped%29.jpg"
HERO_SIZE=$(wc -c < artonly.io/assets/images/artists/future.jpg)
echo "future.jpg: $HERO_SIZE bytes"
if [ "$HERO_SIZE" -lt 5000 ]; then echo "ERROR: hero image too small"; exit 1; fi

# Step 4b: Download body images
echo "--- Downloading body image: intro ---"
curl -s -L -A "$UA" \
  -o "artonly.io/assets/images/artists/future-intro.jpg" \
  "https://upload.wikimedia.org/wikipedia/commons/8/84/Future_-_Openair_Frauenfeld_2019_03.jpg"
SZ=$(wc -c < artonly.io/assets/images/artists/future-intro.jpg)
echo "future-intro.jpg: $SZ bytes"
if [ "$SZ" -lt 5000 ]; then echo "ERROR: future-intro.jpg too small ($SZ bytes)"; exit 1; fi

echo "--- Downloading body image: live ---"
curl -s -L -A "$UA" \
  -o "artonly.io/assets/images/artists/future-live.jpg" \
  "https://upload.wikimedia.org/wikipedia/commons/2/24/Future_Summer_Sixteen_Tour.jpg"
SZ=$(wc -c < artonly.io/assets/images/artists/future-live.jpg)
echo "future-live.jpg: $SZ bytes"
if [ "$SZ" -lt 5000 ]; then echo "ERROR: future-live.jpg too small ($SZ bytes)"; exit 1; fi

echo "--- Downloading body image: studio ---"
curl -s -L -A "$UA" \
  -o "artonly.io/assets/images/artists/future-studio.jpg" \
  "https://upload.wikimedia.org/wikipedia/commons/1/18/JD_Sports_-_Global_Access_is_Here_ft._-MetroBoomin_JD_Sports_US_%28sfn-ALKUQmU_-_0m08s%29_%28cropped%29.jpg"
SZ=$(wc -c < artonly.io/assets/images/artists/future-studio.jpg)
echo "future-studio.jpg: $SZ bytes"
if [ "$SZ" -lt 5000 ]; then echo "ERROR: future-studio.jpg too small ($SZ bytes)"; exit 1; fi

echo "--- Downloading body image: cover ---"
curl -s -L -A "$UA" \
  -o "artonly.io/assets/images/artists/future-cover.jpg" \
  "https://upload.wikimedia.org/wikipedia/commons/f/fd/Future_%28rapper%29_2_2014.JPG"
SZ=$(wc -c < artonly.io/assets/images/artists/future-cover.jpg)
echo "future-cover.jpg: $SZ bytes"
if [ "$SZ" -lt 5000 ]; then echo "ERROR: future-cover.jpg too small ($SZ bytes)"; exit 1; fi

# Step 5: Count words then publish
echo "--- Publishing Future article ---"
python3 << 'PYEOF'
...
python3 artonly.io/agent/make-social-card.py future \
  "Future: The Record That Broke Spotify and the Mixtape That Ignored Everything" 0
...
echo "=== Future deploy complete: $(date) ==="
